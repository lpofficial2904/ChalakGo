...
router.post("/", async (req, res) => {
  ...
  try {
    ...
    try {
      emailSent = await sendContactMessage(message.toObject());
    } catch (emailError) {
    ...
    try {
      ...
      whatsappSent = await sendWhatsAppText(
        `New ChalakGo contact enquiry\n\nName: ${item.name}\nPhone: ${item.phone}\nEmail: ${item.email}\nMessage: ${item.message}`,
      );
    } catch (whatsappError) {
      console.error(
        "Contact WhatsApp notification failed:",
        whatsappError.message,
      );
    }
    res.status(201).json({ message, emailSent, whatsappSent });
  } catch (error) {
