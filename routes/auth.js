...
import { clearAdminSession, createAdminSession, requireAdmin } from '../middleware/auth.js'
...
router.get('/status', async (_req, res) => {
  if (process.env.ADMIN_USERNAME && process.env.ADMIN_PASSWORD) return res.json({ setupRequired: false, environmentLogin: true })
  if (!isDatabaseConnected()) return res.status(503).json({ message: 'Database is not connected.' })
  res.json({ setupRequired: (await Admin.countDocuments()) === 0, environmentLogin: false })
})
...
router.post('/setup', async (req, res) => {
  ...
  if (!/^\d{10}$/.test(mobile) || !validPassword(password)) return res.status(400).json({ message: 'Enter a valid 10-digit mobile number and a password of at least 8 characters.' })
  if (await Admin.countDocuments()) return res.status(409).json({ message: 'An admin account already exists. Please log in.' })
  const admin = await Admin.create({ mobile, passwordHash: await bcrypt.hash(password, 12) })
...
router.post('/logout', (_req, res) => { clearAdminSession(res); res.status(204).end() })
