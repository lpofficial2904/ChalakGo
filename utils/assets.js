const publicApiBase = (process.env.PUBLIC_API_URL || 'https://chalakgo.onrender.com').replace(/\/$/, '')
...
function normalizeAssetUrl(url) {
  ...
  return url.replace(/^https?:\/\/(?:localhost|127\.0\.0\.1)(?::\d+)?(\/uploads\/[^?#]+)([?#].*)?$/i, (_match, assetPath, suffix = '') => `${publicApiBase}${assetPath}${suffix}`)
}
