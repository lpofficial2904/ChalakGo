...
test("temporary-driver pricing matches the published daily examples", () => {
  ...
  for (const [hours, expected] of [
    ...
    const endDateTime = new Date(
      Date.parse(`${startDateTime}:00Z`) + hours * 60 * 60 * 1000,
    )
      .toISOString()
      .slice(0, 16);
...
test("temporary-driver pricing has a 10-hour base charge and a one-time discount", () => {
  const fare = calculateTemporaryDriverFare({
    startDateTime: "2026-09-08T10:00",
    endDateTime: "2026-09-09T10:00",
  });
...
test("Cab vehicle rates calculate SUV and Hatchback fares by distance", () => {
  assert.equal(
    calculateDistanceFare({
      distanceKm: 100,
      carType: "SUV",
      vehicleRates: { suv: 18, hatchback: 14 },
    }).totalFare,
    3500,
  );
  assert.equal(
...
test("Permanent Driver shift rates calculate monthly estimates", () => {
  ...
  assert.equal(
    calculateMonthlyFare({ duration: "6–8 Hours / Day", monthlyRates })
      .totalFare,
    15000,
  );
  assert.equal(
    calculateMonthlyFare({ duration: "8–10 Hours / Day", monthlyRates })
      .totalFare,
    18000,
  );
  assert.equal(
    calculateMonthlyFare({ duration: "10–12 Hours / Day", monthlyRates })
      .totalFare,
    22000,
  );
});
