...
import { addressFormValues, pickupPayload } from '../../frontend/src/utils/location.js'
import Booking from '../models/Booking.js'

test('endpoint coordinates reject missing, blank, array and out of range values', () => {
  for (const value of [undefined, null, '', ' ', ['1'], 'NaN', '91']) assert.throws(() => parseCoordinate(value, 90))
...
test('uses precise coordinates, identifies app, coalesces and caches lookups', async () => {
  let calls = 0
  let resolveResponse
  const geocode = createReverseGeocoder({ fetchImpl: async (url, options) => {
    calls++
  ...
  resolveResponse({ ok: true, json: async () => ({ display_name: 'Test address' }) })
...
test('rate limiting and cache expiry do not reuse neighbouring coordinates', async () => {
  let time = 1000
  let calls = 0
  const geocode = createReverseGeocoder({ now: () => time, fetchImpl: async () => { calls++; return { ok: true, json: async () => ({ display_name: 'Test address' }) } } })
...
test('upstream errors and unmapped coordinates fail without fabricated data', async () => {
  for (const response of [{ ok: false, status: 503 }, { ok: true, json: async () => ({ error: 'Unable to geocode' }) }]) {
...
test('booking schema preserves edited structured address and coordinates', async () => {
  const form = { ...addressFormValues(null), address: 'Corrected address', houseNumber: '8', buildingName: 'Test Apartments',
    mainRoad: 'Test Street', neighbourhood: 'Test Neighbourhood', suburb: 'Test Suburb', locality: 'Test Locality', district: 'Test District', pincode: '302001' }
  const booking = new Booking({ ...form, ...pickupPayload(form, 'current', { latitude: 12.3456789, longitude: 76.5432198, accuracy: 12 }, 123456),
    fullName: 'Test User', phone: '9876543210', email: 'test@example.com', service: 'Driver Only', carType: 'Sedan', startDateTime: '2026-09-08T13:11', endDateTime: '2026-09-08T19:12' })
