...
export function createReverseGeocoder({
  ...
  return async (latitude, longitude) => {
    ...
    if (busy || now() < nextRequestAt)
      throw Object.assign(
        new Error("Address lookup is busy. Please retry in a few seconds."),
        { status: 429 },
      );
    busy = true;
    ...
    const request = (async () => {
      ...
      const response = await fetchImpl(url, {
        headers: {
          "User-Agent":
            ...
            "ChalakGo/1.0 (https://chalakgoo.netlify.app/contact)",
      ...
      if (!response.ok) {
        ...
        throw Object.assign(
          new Error(
            "Address lookup is unavailable. Please retry or enter the address manually.",
          ),
      ...
      if (
        ...
        throw Object.assign(
          new Error(
            "No mapped address was found for these coordinates. Enter the address manually.",
          ),
          { status: 404 },
        );
