...
import { calculateDistanceFare, calculateFixedFare, calculateMonthlyFare, calculateTemporaryDriverFare } from '../utils/temporaryDriverFare.js'
...
const bookingSchema = new mongoose.Schema({
  ...
  pickupPincode: { type: String, trim: true, match: [/^$|^[1-9][0-9]{5}$/, 'Enter a valid 6-digit pincode'] },
  ...
  locationSource: { type: String, enum: ['gps', 'manually_adjusted_pin', 'manual'] },
  ...
  monthlyRates: { sixToEight: Number, eightToTen: Number, tenToTwelve: Number },
  tourPlanDays: Number,
...
bookingSchema.pre('validate', function () {
  ...
  try {
    const fare = this.pricingType === 'distance'
