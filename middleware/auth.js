...
const secret = () => process.env.JWT_SECRET || 'development-only-change-this-jwt-secret'
...
export function createAdminSession(res, admin) {
  ...
  res.cookie(cookieName, token, { httpOnly: true, sameSite: 'lax', secure: process.env.NODE_ENV === 'production', maxAge })
  return token
}
...
export function clearUserSession(res) { res.clearCookie('chalakgo_user_session', { httpOnly: true, sameSite: 'lax', secure: process.env.NODE_ENV === 'production' }) }
